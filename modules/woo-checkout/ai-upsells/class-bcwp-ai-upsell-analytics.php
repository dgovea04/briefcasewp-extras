...
class BEWIA_AI_Upsell_Analytics {
	...
	public static function build_attribution_match( $identity ) { $identity = self::normalize_offer_identity( $identity ); return $identity['session_id'] . '|' . $identity['campaign_key'] . '|' . $identity['variant_id'] . '|' . $identity['offer_id'] . '|' . $identity['product_id']; }
