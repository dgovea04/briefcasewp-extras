...
if ( ! class_exists( 'Bew_Builder_Templates_Manager' ) ) {

	/**
	 * Define Bew_Builder_Templates_Manager class
	 */
	class Bew_Builder_Templates_Manager {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_bew_builder_get_templates', array( $this, 'get_templates' ) );
			add_action( 'wp_ajax_bew_builder_clone_template', array( $this, 'clone_template' ) );

			// Process BewImpext template request
			if ( defined( 'ELEMENTOR_VERSION' ) && version_compare( ELEMENTOR_VERSION, '2.2.8', '>' ) ) {
				add_action( 'elementor/ajax/register_actions', array( $this, 'register_ajax_actions' ), 9 ); //cambiado por que no podia insertar template de 20 a 9
			} else {
			...
			add_filter( 'bew-builder/assets/editor/localize', array( $this, 'localize_tabs' ) );
			add_filter( 'bew-builder/assets/editor/localize', array( $this, 'pass_license_data_to_editor' ) );
		...
		public function localize_tabs( $data ) {
			...
			//	if ( bew_builder()->templates->slug() === $screen->post_type && 'post' === $screen->base ) {
		...
		public function pass_license_data_to_editor( $data ) {
			...
			$license = Bew_Builder_Utils::get_bew_builder_license();			
			$link    = sprintf(
				'<a class="template-library-activate-license" href="%1$s" target="_blank">%2$s %3$s</a>',
				Bew_Builder_Utils::active_license_link(),
		...
		public function register_sources() {
			...
			require BEW_EXTRAS_PATH . 'includes/bew-builder/includes/templates/sources/base.php';

			$sources = array(
				'bew-api'   => 'Bew_Builder_Templates_Source_Api',
				'bew-local' => 'Bew_Builder_Templates_Source_Local',
			);

			foreach ( $sources as $source => $class ) {
				require BEW_EXTRAS_PATH . 'includes/bew-builder/includes/templates/sources/' . $source . '.php';
		...
		public function register_library_types() {
			...
			$this->_library_types = bew_builder()->structures->get_structures_for_post_type();
		...
		public function get_template_tabs() {
			...
			$tabs = bew_builder()->structures->get_structures_for_popup();

			//$tabs = array_merge( $tabs, array(
			//	'local' => array(
			//		'title'    => esc_html__( 'My Library', 'bew-extras' ),
			//		'data'     => array(),
			//		'sources'  => array( 'bew-local' ),
			//		'settings' => array(),
			//	),
			//) );
		...
		public function get_templates_list() {
			...
			$templates = get_posts( array(
				'posts_per_page' => -1,
				'post_status'    => 'publish',
				'post_type'      => bew_builder()->templates->slug(),
			) );
		...
		public function get_templates_for_options() {
			...
			$default = array(
				'0' => esc_html__( 'Not Selected', 'bew-extras' ),
			);
		...
		public function get_templates() {
			...
			$all_cats = array(
				array(
					'slug' => '',
					'title' => esc_html__( 'All', 'bew-extras' ),
				)
			);			
		...
		public function clone_template() {
			...
			if ( ! empty( $template_data['content'] ) ) {
				wp_insert_post( array(
					...
					'tax_input'  => array(
						bew_builder()->templates->type_tax => $template_data['type'],
					),
				) );
			}
		...
		public function register_ajax_actions( $ajax ) {
			...
			// saque por que tenia incompatilidad con guardar template en elementor
